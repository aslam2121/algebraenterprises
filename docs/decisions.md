...
- Local one-off Strapi scripts that target Render Postgres should tolerate the known `tarn` shutdown `aborted` error and keep the successful write summary, because the failure can happen during `strapi.destroy()` or as a late Node 22 `unhandledRejection` after the actual work is finished
