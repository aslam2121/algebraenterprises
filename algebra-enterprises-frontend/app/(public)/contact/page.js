    ...
    <main style={{ minHeight: '100vh', background: 'var(--navy)', paddingTop: '80px' }}>
      <section style={{ padding: '4rem 0 2rem' }}>
        <div className="container" style={{ display: 'grid', gap: '1rem' }}>
          ...
          <p style={{ color: 'var(--muted)', maxWidth: '640px', lineHeight: 1.8 }}>
          ...
          <div style={{ background: 'rgba(17,34,64,0.75)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '20px', padding: '1.8rem' }}>
            ...
            <div style={{ display: 'grid', gap: '0.9rem' }}>
              {CONTACT_ITEMS.map(([label, value]) => (
                <div key={label} style={{ paddingBottom: '0.9rem', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                  <div style={{ fontSize: '0.72rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.3rem' }}>
                    {label}
                  </div>
                  <div style={{ color: '#fff', lineHeight: 1.6 }}>{value}</div>
                </div>
              ))}
            </div>
          </div>

          <div style={{ background: 'linear-gradient(135deg, rgba(17,34,64,0.82), rgba(26,58,92,0.66))', border: '1px solid rgba(201,168,76,0.15)', borderRadius: '20px', padding: '1.8rem' }}>
            <p style={{ fontSize: '0.76rem', color: 'var(--gold)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1rem' }}>
              We Can Help With
            </p>
            <div style={{ display: 'grid', gap: '0.8rem', marginBottom: '1.6rem' }}>
              {SUPPORT_AREAS.map((item) => (
                <div key={item} style={{ padding: '0.95rem 1rem', borderRadius: '14px', background: 'rgba(10,22,40,0.55)', color: '#d5dfef', lineHeight: 1.7 }}>
                  {item}
                </div>
              ))}
            </div>
            <div style={{ display: 'flex', gap: '0.8rem', flexWrap: 'wrap' }}>
              <Link href="/properties" style={{ display: 'inline-block', textDecoration: 'none', padding: '0.8rem 1.4rem', borderRadius: '10px', background: 'linear-gradient(135deg, var(--red), var(--red-light))', color: '#fff', fontWeight: 600 }}>
                Browse Properties
              </Link>
