...
export default function AboutPage() {
  return (
    <main style={{ minHeight: '100vh', background: 'var(--page-bg)', paddingTop: '80px' }}>
      <section style={{ padding: '4rem 0 2rem', borderBottom: '1px solid var(--line-gold)' }}>
        <div className="container" style={{ display: 'grid', gap: '1.5rem' }}>
          ...
          <p style={{ color: 'var(--text-muted)', maxWidth: '680px', lineHeight: 1.8, fontSize: '1rem' }}>
      ...
      <section style={{ padding: '3rem 0' }}>
        <div className="container" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, rgba(192,57,43,0.12), rgba(201,168,76,0.12))', border: '1px solid var(--line-gold)', borderRadius: '20px', padding: '1.8rem' }}>
            <p style={{ fontSize: '0.76rem', color: 'var(--gold)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '0.9rem' }}>
              Focus Areas
            </p>
      ...
      <section style={{ padding: '1rem 0 3rem' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
            {PRINCIPLES.map((item) => (
              <article key={item.title} style={{ background: 'var(--surface-2)', border: '1px solid var(--line-soft)', borderRadius: '18px', padding: '1.5rem' }}>
                <h2 style={{ fontSize: '1.25rem', marginBottom: '0.8rem' }}>{item.title}</h2>
                <p style={{ color: 'var(--text-muted)', lineHeight: 1.75 }}>{item.body}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section id="team" style={{ padding: '0 0 4rem' }}>
        <div className="container" style={{ background: 'var(--surface-2)', border: '1px solid var(--line-soft)', borderRadius: '24px', padding: '2rem' }}>
          <p style={{ fontSize: '0.76rem', color: 'var(--gold)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '0.9rem' }}>
            Team
          </p>
          ...
          <div style={{ display: 'grid', gap: '0.8rem' }}>
            {TEAM.map((item) => (
              <div key={item} style={{ padding: '1rem 1.1rem', borderRadius: '14px', background: 'var(--surface-3)', color: 'var(--text-secondary)', lineHeight: 1.7 }}>
                {item}
              </div>
            ))}
          </div>
          <div style={{ marginTop: '1.5rem' }}>
            <Link href="/contact" style={{ display: 'inline-block', textDecoration: 'none', padding: '0.8rem 1.4rem', borderRadius: '10px', background: 'linear-gradient(135deg, var(--red), var(--red-light))', color: 'var(--white)', fontWeight: 600 }}>
              Contact The Team
            </Link>
          </div>
        </div>
